# 3dmk-photo-package-import-b81f implementation handoff

This archive is the source of truth for turning the design into production code. Start from `image-assisted-package-refinement.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `image-assisted-package-refinement.html`
...
- A single self-contained `image-assisted-package-refinement.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `image-assisted-package-refinement.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
## Entry points
- `image-assisted-package-refinement.html`
...
1. Inspect `image-assisted-package-refinement.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
