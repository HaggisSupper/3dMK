# 3dmk-ui-ux-20260717 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `3dmk-workspace.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `3dmk-workspace.html`
...
- A single self-contained `3dmk-workspace.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `3dmk-workspace.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `3dmk-workspace.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
