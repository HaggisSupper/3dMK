    ...
    fn validate(self) -> Result<Self, IntelligenceContractError> {
        ...
        for route in &self.operations {
            if !seen.insert(route.operation) {
                return Err(IntelligenceContractError::DuplicateOperation(
                    route.operation,
                ));
            }
...
pub fn embedded_contract() -> Result<LocalIntelligenceRoutingContract, IntelligenceContractError> {
    parse_contract(include_str!(
        "../contracts/local-intelligence-routing.v1.json"
    ))
}
...
mod tests {
    ...
    fn contract_rejects_silent_fallback_drift() {
        let invalid = include_str!("../contracts/local-intelligence-routing.v1.json").replace(
            "\"llamacpp_automatic_fallback\": false",
            "\"llamacpp_automatic_fallback\": true",
        );
        assert!(matches!(
    ...
    fn mistralrs_is_primary_and_llamacpp_is_explicit_only() {
        let decision =
            evaluate_request(&request(IntelligenceOperation::VisionAdjudication)).unwrap();
